...
const MultiSelect = ({
  ...
  const handleKeyDown = (e) => {
    ...
    // e.preventDefault() is often used here, but be careful with native scrolling
    switch (e.key) {
      case "Enter":
        e.preventDefault();
        if (!isOpen) {
              ...
              <div className="flex flex-wrap flex-auto gap-2">
                {selectedOptions.length > 0 ? (
                  selectedOptions.map((v) => {
                    ...
                    const text = option ? option.text : v;
                    return (
                      <div
                        key={v}
            ...
            >
              {options.map((option, index) => {
                ...
                return (
                  <div
                    key={option.value}
                    className={`hover:bg-gray-100 dark:hover:bg-gray-800 w-full cursor-pointer rounded-t border-b border-gray-200 dark:border-gray-800 ${
