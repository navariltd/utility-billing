...
class UtilityServiceRequest(Document):
	...
	def validate_items(self):
		"""Fill the items table from the bill structure when it is empty.

		The table is optional: a request may be saved without items and the
		items added later (for example on the Sales Order built from it). When
		a Utility Bill Structure is set, its items are used to populate the
		table as before.
		"""
		if self.items or not self.utility_bill_structure:
			return
	...
	def validate_contract_dates(self):
		...
		if self.start_date and self.contract_length_months and not self.end_date:
			...
			self.contract_length_months = self.get_month_diff(
	...
	def validate_child_items(self):
		...
		for row in self.requested_properties:
			...
			if row_start and parent_start and row_start < parent_start:
				frappe.throw(
					f"Property start date '{row_start}' in row {row.idx} cannot be before contract start date '{parent_start}'."
				)

			if row_end and parent_end and row_end > parent_end:
				frappe.throw(
					f"Property end date '{row_end}' in row {row.idx} cannot be after contract end date '{parent_end}'."
				)
...
def ensure_service_items_for_properties(doc) -> None:
	...
	"""
	properties = [row.utility_property for row in doc.requested_properties if row.utility_property]
...
def get_site_survey_defaults(docname):
	...
	request_type_description = frappe.db.get_value("Issue Type", doc.request_type, "description")

	description = f"Site survey for Utility Service Request: {docname}\nCustomer: {doc.customer_name}\n"
...
def update_request_status(request_name):
	issues = frappe.get_list(
		"Issue", filters={"utility_service_request": request_name}, pluck="status", ignore_permissions=1
	)

	submitted_boms = frappe.get_list(
		"BOM", filters={"utility_service_request": request_name}, pluck="docstatus", ignore_permissions=1
	)
...
def get_utility_bill_structure_details(name):
	...
	return {"items": items, "dimensions": dimensions}
...
def create_sales_order_doc(
	...
	usr, property_line, final_transaction_date, final_start_date, final_end_date = _get_common_doc_details(
		docname, property, transaction_date, start_date, end_date
	)
	...
	so = frappe.get_doc(
		{
			...
			if hasattr(usr, "payment_terms_template")
			else None,
	...
	_handle_auto_repeat(
		so, usr, property_line, enable_auto_repeat, adjustment_rule, final_start_date, final_end_date
	)
...
def create_sales_invoice_doc(
	...
	"""
	items = _validate_items(items)
	usr, property_line, final_posting_date, final_start_date, final_end_date = _get_common_doc_details(
		docname, property, posting_date, start_date, end_date
	)
	...
	_handle_auto_repeat(
		si, usr, property_line, enable_auto_repeat, adjustment_rule, final_start_date, final_end_date
	)
...
def _get_common_doc_details(docname, property, transaction_date, start_date, end_date):
	...
	if property:
		property_line = next(
			(prop for prop in usr.requested_properties if prop.utility_property == property), None
		)
...
def add_transaction_comments(transaction, usr_name, auto_repeat=None):
	...
	def create_comment_content(title, doc, show_property=True, show_request_link=False):
		...
		if show_property and auto_repeat and auto_repeat.get("utility_property"):
			property_name = frappe.db.get_value(
				"Utility Property", auto_repeat.get("utility_property"), "property_name"
			)
			content += f"""
            <br>
	...
	if auto_repeat and auto_repeat.get("utility_property"):
		property_comment = create_comment_content(
			f"{transaction.doctype} Created", transaction, show_property=False, show_request_link=True
		)
...
def _get_property_doc(usr, utility_property):
	...
	if not utility_property:
		return None
	return next(
		(prop for prop in usr.requested_properties if prop.utility_property == utility_property), None
	)
	...
	return frappe.get_doc(
		{
			...
			"increment_interval_months": increment_details["interval"]
...
def _get_increment_details(property_doc, adjustment_rule):
	...
	increment_interval = (
		float(adjustment_rule.increment_interval_months)
		if property_doc and adjustment_rule.increment_interval_months
		else 0
	)
	increment_percent = (
		float(adjustment_rule.increment_percentage)
		if property_doc and adjustment_rule.increment_percentage
		else 0
	)
...
def _calculate_dates(auto_repeat, increment_details):
	...
	return {
		...
		"increment_date": increment_date,
		"contract_end_date": contract_end_date,
		"first_increment_period": increment_details["total_first_period"]
		if increment_details["has_increment"]
		else None,
	}
...
def _calculate_end_date(
	...
	if frequency == "Monthly":
		...
		if years_before > 0:
			...
			base_end_date = base_end_date.replace(
				day=min(int(repeat_on_day or start_date.day), get_last_day(base_end_date).day)
			)
		else:
			base_end_date = add_days(increment_date, -1)
