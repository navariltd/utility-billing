...
def get_columns():
	return [
		...
		{"label": _("Remaining Days"), "fieldname": "remaining_days", "fieldtype": "Int", "width": 120},
		{
...
def get_data(filters):
	...
	if filters.get("company"):
		service_requests = frappe.get_all(
			"Utility Service Request", filters={"company": filters.get("company")}, fields=["name"]
		)
		...
		contracts_with_sr = frappe.get_all(
			"Contract", filters={"utility_service_request": ["in", sr_names], "docstatus": 1}, fields=["name"]
		)
	...
	for contract in contracts:
		...
		for idx, prop in enumerate(properties):
			if (
				...
				and prop.end_date
				and getdate(prop.end_date) < getdate(filters.get("from_date"))
			):
				continue
			if (
				filters.get("to_date")
				and prop.start_date
				and getdate(prop.start_date) > getdate(filters.get("to_date"))
			):
				continue
...
def get_report_summary(data):
	...
	summary.insert(
		0, {"value": total, "indicator": "Blue", "label": _("Total Properties"), "datatype": "Int"}
	)
...
def get_chart_data(data):
	...
	chart = {
		"data": {
			"labels": labels,
			"datasets": [{"name": _("Contract Status"), "values": list(status_data.values())}],
		},
		"type": "pie",
		"height": 300,
		"colors": colors,
