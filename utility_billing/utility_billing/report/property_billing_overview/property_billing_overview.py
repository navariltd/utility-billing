...
class PropertyBillingOverview:
	...
	def get_contract_data(self):
		...
		if self.filters.get("property"):
			query = query.where(contract_property.utility_property == self.filters.property)
	...
	def build_bill_item_mapping(self):
		bill_structure_names = list(
			set(c.utility_bill_structure for c in self.contracts if c.utility_bill_structure)
		)
		...
		bill_structure_items = (
			...
			.select(ubsi.item, item.item_group, ubsi.parent.as_("utility_bill_structure"))
	...
	def get_opening_balances(self, customer_list):
		for customer in customer_list:
			...
			for contract in (c for c in self.contracts if c.customer == customer):
				self.billing_data[(contract.property, contract.customer)]["opening_outstanding"] = (
					opening_balance
				)
	...
	def get_paid_in_period_by_property(self, property_list, customer_list):
		...
		query = (
			...
			.select(ple.party, ple.against_voucher_no, ple.against_voucher_type, ple.amount)
		...
		for payment in payment_entries:
			...
			if voucher_type == "Sales Invoice":
				invoice = (
					frappe.qb.from_(si).select(si.grand_total).where(si.name == voucher_no).run(as_dict=True)
				)
				...
				for prop_data in invoice_items:
					if prop_data.utility_property in property_list:
						proportion = (
							flt(prop_data.base_amount) / total_invoice_amount if total_invoice_amount else 1
						)
				...
				order = (
					frappe.qb.from_(so).select(so.grand_total).where(so.name == voucher_no).run(as_dict=True)
				)
				...
				for prop_data in order_items:
					if prop_data.utility_property in property_list:
						proportion = (
							flt(prop_data.base_amount) / total_order_amount if total_order_amount else 1
						)
						distributed_payment = payment_amount * proportion
						property_key = (prop_data.utility_property, customer)
						property_payments[property_key] += distributed_payment
	...
	def get_current_outstanding(self, customer_list):
		for customer in customer_list:
			...
			for contract in (c for c in self.contracts if c.customer == customer):
				self.billing_data[(contract.property, contract.customer)]["current_outstanding"] = (
					closing_balance
				)
	...
	def process_data(self):
		...
		for contract in self.contracts:
			...
			row = {
				...
				"contact_number": (contract.contact_number if is_first_row_for_customer else ""),
				...
				"opening_arrears": (
					billing_info.get("opening_outstanding", 0) if is_first_row_for_customer else ""
				),
				"total_paid": billing_info.get("paid", 0),  # Show paid amount for each property/tenancy
				"current_outstanding_arrears": (
					billing_info.get("current_outstanding", 0) if is_first_row_for_customer else ""
				),
		...
		for contract in self.contracts:
			...
			for bt in self.bill_types:
				if bt.name in billing_info["bill_type_invoiced"]:
					customer_data["bill_type_invoiced"][bt.name] += billing_info["bill_type_invoiced"][
						bt.name
					]
	...
	def clean_columns(self):
		...
		for col in self.columns:
			...
			all_zero = all(not row.get(fieldname) or flt(row.get(fieldname)) == 0 for row in self.data)
