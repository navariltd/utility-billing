...
def is_contract_ended(auto_repeat_doc: Document) -> bool:
	...
	if contract_end_date and getdate(contract_end_date) < getdate(today()):
		frappe.msgprint(
			_("Contract ended on {0}. Cannot generate renewal.").format(
...
def get_document_copy(auto_repeat_doc: Document) -> Document:
	...
	return frappe.copy_doc(
		frappe.get_doc(auto_repeat_doc.reference_doctype, auto_repeat_doc.reference_document)
	)
...
def apply_rate_adjustments(
	auto_repeat_doc: Document, reference_doc: Document, adjustment_rule: Document
) -> None:
	...
	if hasattr(reference_doc, "items"):
		for item in reference_doc.items:
			...
			original_rate = get_original_item_rate(
				auto_repeat_doc.reference_doctype, auto_repeat_doc.reference_document, item.item_code
			)
...
def calculate_new_rate(
	current_rate: float, original_rate: float | None, increment_percent: float, adjustment_rule: Document
) -> float:
...
def get_original_item_rate(doctype: str, docname: str, item_code: str) -> float | None:
...
def create_auto_repeat(
	original_auto_repeat: Document, new_reference_doc: Document, adjustment_rule: Document
) -> Document:
	...
	new_auto_repeat = frappe.get_doc(
		{
			...
			"end_date": get_auto_repeat_end_date(
				original_auto_repeat, original_auto_repeat.end_date, adjustment_rule
			),
...
def get_auto_repeat_end_date(
	auto_repeat_doc: Document, start_date: str, adjustment_rule: Document
) -> str | None:
	...
	proposed_end_date = (
		add_months(start_date_obj, flt(adjustment_rule.increment_interval_months))
		if adjustment_rule.increment_interval_months
		else None
	)
...
def add_audit_comment(
    ...
    """).format(
		...
		start=formatdate(original_auto_repeat.end_date)
		if hasattr(original_auto_repeat, "end_date")
		else _("N/A"),
		...
		old_ar_link=(
			f"<a href='/app/auto-repeat/{original_auto_repeat.name}'>{original_auto_repeat.name}</a>"
			if original_auto_repeat
			else _("N/A")
		),
...
def run_all_due_auto_repeats():
	...
	names = frappe.get_all(
		"Auto Repeat", filters={"disabled": 0, "next_schedule_date": ["<=", today_date]}, pluck="name"
	)
