...
def enqueue_sales_invoice_creation(source_names):
	...
	if not isinstance(source_names, (list, tuple)):
		raise ValueError(_("Invalid input type for source_names. Expected JSON array, list, or tuple."))
	...
	return _("Started a background job to create Sales Invoice(s) for {0} customer(s).").format(num_customers)
...
def create_sales_invoices_in_background(source_names):
	...
	for customer, orders in orders_by_customer.items():
		try:
			...
			frappe.msgprint(_("Successfully created Sales Invoice for customer: {0}").format(customer))
		except Exception as e:
			for order in orders:
				create_log(order, e, "Sales Order", "Sales Invoice", "Failed")
			frappe.log_error(f"Error creating Sales Invoice for customer {customer}, {e}")
			frappe.msgprint(_("Error creating Sales Invoice for customer {0}: {1}").format(customer, e))
			continue
...
def make_sales_invoice(source_names, target_doc=None, ignore_permissions=False):
	...
	for source_name in source_names:
		try:
			current_doc = map_sales_order_to_invoice(source_name, target_doc, ignore_permissions)
...
def log_error_and_continue(source_name, e):
	...
	frappe.log_error(message=str(e), title=f"Error processing Sales Order {source_name}")
