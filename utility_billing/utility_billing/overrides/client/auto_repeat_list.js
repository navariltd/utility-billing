	...
	onload: function (listview) {
		listview.page.add_action_item(
			...
			function () {
				...
				if (!selected.length) {
					frappe.msgprint(
						__("Please select at least one Auto Repeat"),
					);
					return;
				}
				...
				frappe.call({
					...
					callback: function (r) {
						if (!r.exc) {
							frappe.msgprint(
								__(
									"Selected Auto Repeats processed successfully",
								),
							);
							listview.refresh();
						}
		...
		listview.page.add_inner_button(
			__("Run All Due Auto Repeats"),
			function () {
				frappe.confirm(
					__("Are you sure you want to run all due Auto Repeats?"),
					function () {
						frappe.call({
							...
							callback: function (r) {
								if (!r.exc) {
									frappe.msgprint(
										__(
											"All due Auto Repeats processed successfully",
										),
									);
									listview.refresh();
								}
