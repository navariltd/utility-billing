...
def clear_bill_structures() -> None:
	docs = frappe.get_list(
		"Utility Bill Structure", filters={"company": "Utility and Rental (Demo)"}, fields=["name"]
	)
...
def insert_bill_structures() -> None:
	...
	for structure in bill_structures:
		...
		for item in structure.get("items", []):
			doc.append(
				"items", {"item": item.get("item"), "amount": item.get("amount"), "total": item.get("total")}
			)
...
def get_random_bill_structure() -> str:
	...
	structures = frappe.get_list(
		"Utility Bill Structure", filters={"docstatus": 1}, fields=["name"], limit=1, order_by="RAND()"
	)
	return structures[0].name if structures else None


def assign_properties_to_request(
	doc, requested_props: list[dict[str, Any]], service_start, service_end, is_signed: bool
) -> None:
	...
	def get_random_insurance():
		...
		insurances = frappe.get_list("Insurance", filters={}, fields=["name"], limit=1, order_by="RAND()")
...
def insert_service_requests() -> None:
	...
	for request in service_requests:
		try:
			...
			assign_properties_to_request(
				doc, request.get("requested_properties", []), service_start, service_end, is_signed
			)
...
def clear_service_requests() -> None:
	...
	demo_requests = frappe.get_all(
		"Utility Service Request", filters={"party_name": ["in", demo_parties]}, fields=["name", "docstatus"]
	)

	for req in demo_requests:
		invoices = frappe.get_all(
			"Sales Invoice", filters={"utility_service_request": req.name}, fields=["name", "docstatus"]
		)
	...
	for req in demo_requests:
		try:
			...
			frappe.log_error(
				title="Service Request Deletion Failed", message=f"Request: {req.name}\nError: {str(e)}"
			)
